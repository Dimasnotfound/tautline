...
func TestV2100VersionAndWidgetRevision(t *testing.T) {
	if appVersion != "2.10.0" {
		t.Fatalf("appVersion=%q, want 2.10.0", appVersion)
	}
...
func TestWorkflowInstructionsRequireSkillMatching(t *testing.T) {
	...
	for _, expected := range []string{"skills_search creates and renders", "do not also call tautline_activity", "trivial status check", "prompt-boundary tools archive the previous monitor", "skills_search", "skill_view", "Before every non-trivial task", "never expose secret configuration values", "workspace_lookup", "mode=worktree", "base_ref", "read_many", "tautline_doctor", "exec_command", "write_stdin", "process session ID", "chatgpt-relay", "worker_prompt", "Laju Relay Bridge", "private consumer ChatGPT backend", "claim_agent_task", "complete_agent_task"} {
