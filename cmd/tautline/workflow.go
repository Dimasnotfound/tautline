...
func codingWorkflowInstructions() string {
	...
	if activeWidgetMode != widgetModeOff {
		return base + " After the final file modification in a turn, call show_changes exactly once before the final response. Do not call show_changes after every edit and do not call it for read-only work. The existing Tautline activity widget updates automatically; do not expect a new widget from each tool call."
	}
	return base
}
